...
class CNN:
    ...
    def TRAIN(self, X_train, y_train, epochs=200, LearningRate=0.01, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            acc_train = accuracies.append(np.mean(batch_accuracies))
            if epoch % 10 == 0:
...
def TRAIN_CNN(TESTING_DIRECTORY):
...
def test_button(model_path, TESTING_DIRECTORY):
    test_dir = TESTING_DIRECTORY
    TEST_CNN("parameters.npz", test_dir)

def train_button(model_path, TESTING_DIRECTORY):
    cnn, losses, accuracies, test_accuracy = TRAIN_CNN(TESTING_DIRECTORY)
...
#   ------------------------------------------------ TEST BELOW ------------------------------------------------ #
    ...
    test_button("parameters.npz", images_directory)

#  ------------------------------------------------ TRAIN BELOW ------------------------------------------------ #

    # test_dir = "C:/Users/ADMIN/Downloads/nermeen_test_w/shape_dataset_final_bw_white_bg"
    # TEST_CNN("parameters.npz", test_dir)
